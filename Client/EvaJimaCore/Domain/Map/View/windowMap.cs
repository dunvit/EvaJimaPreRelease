...
{
    public partial class WindowMap : Form, IMapView
    {
        ...
        private Point drugAndDropStartPosition = new Point(0, 0);
        ...
        private void Map_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if(e.Button == MouseButtons.Left)
            {
                ...
                isDragging = true;
                drugAndDropStartPosition = new Point((Width / 2) - e.X, (Height / 2) - e.Y);
        ...
        private void Event_MapClick(object sender, MouseEventArgs e)
        {
        ...
        public void DrawSpaceMap(List<SolarSystem> systems, string centreSystem)
        {
            if(centreSystem == "unknown") return;

            

            ScreenCenter = systems.FirstOrDefault(system => system.Name == centreSystem).LocationInMap;
        ...
        private void WindowMap_Load(object sender, EventArgs e)
        {
            if(SendMessage != null) SendMessage(null);
        ...
        private void Event_OnPaint(object sender, PaintEventArgs e)
        {
            ...
            try
            {
                ...
                foreach(var solarSystem in Global.Pilots.Selected.SpaceMap.Systems)
                {
                    if (solarSystem.Name == null || solarSystem.Name != Global.Pilots.Selected.SpaceMap.SelectedSolarSystemName) continue;
                ...
                foreach (var solarSystem in Global.Pilots.Selected.SpaceMap.Systems)
                {
                    if (solarSystem.Name == null || solarSystem.Name != Global.Pilots.Selected.SpaceMap.LocationSolarSystemName) continue;
                ...
                foreach (var solarSystem in Global.Pilots.Selected.SpaceMap.Systems)
                {
                    ...
                    {
                        var connectedSolarSystem = Global.Pilots.Selected.SpaceMap.Systems.FirstOrDefault(system => system.Name == connection);
                        ...
                        var pointTo = new Point(connectedSolarSystem.LocationInMap.X - MapPosition.X, connectedSolarSystem.LocationInMap.Y - MapPosition.Y);
                ...
                foreach (var solarSystem in Global.Pilots.Selected.SpaceMap.Systems)
                {
                    ...
                    if (_systemsInformation.ContainsKey(solarSystem.Name) == false) _systemsInformation.Add(solarSystem.Name, Global.Space.GetSolarSystem(solarSystem.Name));
                    ...
                    {
                        e.Graphics.DrawString(solarSystem.Name, drawFont, drawBrushName, solarSystem.LocationInMap.X - MapPosition.X + 2 - stringSize.Width / 2, solarSystem.LocationInMap.Y - MapPosition.Y - 30, drawFormat);
                    }
                    ...
                    if (Tools.IsWSpaceSystem(solarSystem.Name))
                    {
                        ...
                        e.Graphics.DrawString("[C" + _systemsInformation[solarSystem.Name].Class + "]", drawFont, drawBrush,
                            solarSystem.LocationInMap.X - MapPosition.X + 2 - stringSize.Width / 2 + stringSize2.Width, solarSystem.LocationInMap.Y - MapPosition.Y - 30, drawFormat);
                    }

                    var rectangle = new Rectangle(solarSystem.LocationInMap.X - MapPosition.X - 8, solarSystem.LocationInMap.Y - MapPosition.Y - 8, 16, 16);

                    e.Graphics.FillEllipse(new SolidBrush(Tools.GetColorBySolarSystem(_systemsInformation[solarSystem.Name].Security.ToString())), rectangle);
        ...
        {
            //RecalculateOffsetPositions(new Point(MapPosition.X + Width / 2, MapPosition.Y + Height / 2));
